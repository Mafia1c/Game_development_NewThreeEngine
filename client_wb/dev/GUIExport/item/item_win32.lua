...
function ui.init(parent, __data__, __update__)
	...
	-- Create Node
	local Node = GUI:Node_Create(parent, "Node", 568, 320)
	GUI:setChineseName(Node, "物品框_节点")
	GUI:setTag(Node, -1)

	-- Create Panel_bg
	local Panel_bg = GUI:Layout_Create(Node, "Panel_bg", 0, 0, 32, 32, false)
	...
	-- Create Image_bg
	local Image_bg = GUI:Image_Create(Panel_bg, "Image_bg", 16, 16, "res/private/item_tips/1900025000.png")
	...
	local Node_sfx_under = GUI:Node_Create(Panel_bg, "Node_sfx_under", 16, 16)
	GUI:setChineseName(Node_sfx_under, "物品框_特效层(分前后)")
	GUI:setTag(Node_sfx_under, 22)
	TAGOBJ["22"] = Node_sfx_under

	-- Create Button_icon
	local Button_icon = GUI:Button_Create(Panel_bg, "Button_icon", 16, 16, "res/private/gui_edit/Button_Normal.png")
	...
	TAGOBJ["15"] = Button_icon

	-- Create Node_sfx
	local Node_sfx = GUI:Node_Create(Panel_bg, "Node_sfx", 16, 16)
	GUI:setChineseName(Node_sfx, "物品框_特效节点")
	GUI:setTag(Node_sfx, 16)
	TAGOBJ["16"] = Node_sfx

	-- Create Node_lock
	local Node_lock = GUI:Node_Create(Panel_bg, "Node_lock", 16, 16)
	GUI:setChineseName(Node_lock, "物品框_锁节点")
	GUI:setTag(Node_lock, 32)
	TAGOBJ["32"] = Node_lock

	-- Create Text_count
	local Text_count = GUI:Text_Create(Panel_bg, "Text_count", 45, 25, 13, "#ffffff", [[1]])
	GUI:Text_enableOutline(Text_count, "#000000", 2)
	GUI:setChineseName(Text_count, "物品框_数量_文本")
	GUI:setAnchorPoint(Text_count, 1.50, 1.00)
	GUI:setTouchEnabled(Text_count, false)
	GUI:setTag(Text_count, 17)
	TAGOBJ["17"] = Text_count

	-- Create Text_star_lv
	local Text_star_lv = GUI:Text_Create(Panel_bg, "Text_star_lv", 0, 32, 18, "#efad21", [[0]])
	...
	TAGOBJ["28"] = Text_star_lv

	-- Create Panel_extra
	local Panel_extra = GUI:Layout_Create(Panel_bg, "Panel_extra", 16, 16, 32, 32, false)
	...
	TAGOBJ["18"] = Panel_extra

	-- Create Node_sfx_power
	local Node_sfx_power = GUI:Node_Create(Panel_bg, "Node_sfx_power", 16, 16)
	GUI:setChineseName(Node_sfx_power, "物品框_装备对比_特效节点")
	GUI:setTag(Node_sfx_power, 25)
	TAGOBJ["25"] = Node_sfx_power

	-- Create Image_power
	local Image_power = GUI:Image_Create(Panel_bg, "Image_power", 30, 20, "res/public/btn_szjm_01_3.png")
	...
	TAGOBJ["19"] = Image_power

	-- Create Node_needNum
	local Node_needNum = GUI:Node_Create(Panel_bg, "Node_needNum", 34, 2)
	GUI:setChineseName(Node_needNum, "物品框_需求数量节点")
	GUI:setTag(Node_needNum, 20)
	TAGOBJ["20"] = Node_needNum

	-- Create Image_choosTag
	local Image_choosTag = GUI:Image_Create(Panel_bg, "Image_choosTag", 16, 16, "res/public/1900000678_2.png")
	...
	TAGOBJ["26"] = Image_choosTag

	-- Create Node_left_top
	local Node_left_top = GUI:Node_Create(Panel_bg, "Node_left_top", 16, 16)
	GUI:setChineseName(Node_left_top, "物品框_挂接节点(红点等)")
	GUI:setTag(Node_left_top, 29)
	TAGOBJ["29"] = Node_left_top
