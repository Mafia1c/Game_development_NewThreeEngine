...
function ui.init(parent, __data__, __update__)
	...
	-- Create FrameBG
	local FrameBG = GUI:Image_Create(FrameLayout, "FrameBG", 0, 0, "res/custom/npc/42sgdz/bg.png")
	...
	-- Create equipNode1
	local equipNode1 = GUI:Node_Create(midNode, "equipNode1", 360, 411)
	GUI:setTag(equipNode1, 0)

	-- Create equipNode2
	local equipNode2 = GUI:Node_Create(midNode, "equipNode2", 264, 227)
	GUI:setTag(equipNode2, 0)

	-- Create equipNode3
	local equipNode3 = GUI:Node_Create(midNode, "equipNode3", 459, 227)
	GUI:setTag(equipNode3, 0)

	-- Create lockImg
	local lockImg = GUI:Image_Create(midNode, "lockImg", 348, 292, "res/custom/npc/42sgdz/lock.png")
	GUI:setAnchorPoint(lockImg, 0.00, 0.00)
	GUI:setTouchEnabled(lockImg, false)
	GUI:setTag(lockImg, 0)

	-- Create needText1
	local needText1 = GUI:Text_Create(midNode, "needText1", 300, 154, 16, "#ffffff", [[消耗：]])
	GUI:Text_enableOutline(needText1, "#000000", 1)
	GUI:setAnchorPoint(needText1, 0.00, 0.00)
	GUI:setTouchEnabled(needText1, false)
	GUI:setTag(needText1, 0)

	-- Create needText2
	local needText2 = GUI:Text_Create(needText1, "needText2", 44, 0, 16, "#ff0000", [[30000金符]])
	GUI:Text_enableOutline(needText2, "#000000", 1)
	GUI:setAnchorPoint(needText2, 0.00, 0.00)
	GUI:setTouchEnabled(needText2, false)
	GUI:setTag(needText2, 0)

	-- Create upBtn
	local upBtn = GUI:Button_Create(midNode, "upBtn", 364, 118, "res/custom/npc/anb1.png")
	...
	-- Create bigList
	local bigList = GUI:ListView_Create(rightNode, "bigList", 542, 95, 252, 352, 1)
	...
	-- Create list1
	local list1 = GUI:ListView_Create(bigList, "list1", 0, 292, 252, 60, 2)
