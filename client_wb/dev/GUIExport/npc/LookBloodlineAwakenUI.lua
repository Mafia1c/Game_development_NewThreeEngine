...
function ui.init(parent, __data__, __update__)
	...
	-- Create select_node
	local select_node = GUI:Layout_Create(FrameLayout, "select_node", 627, 42, 220, 463, false)
	GUI:setAnchorPoint(select_node, 0.00, 0.00)
	GUI:setTouchEnabled(select_node, false)
	GUI:setTag(select_node, 0)

	-- Create Image_2
	local Image_2 = GUI:Image_Create(select_node, "Image_2", 7, 61, "res/custom/npc/12tmfw/t1.png")
	GUI:setAnchorPoint(Image_2, 0.00, 0.00)
	GUI:setTouchEnabled(Image_2, false)
	GUI:setTag(Image_2, 0)

	-- Create select_item
	local select_item = GUI:ItemShow_Create(select_node, "select_item", 110, 392, {index = 1, count = 1, look = false, bgVisible = false, color = 255, noLockTips = true})
	GUI:setAnchorPoint(select_item, 0.50, 0.50)
	GUI:setTag(select_item, 0)

	-- Create Image_3
	local Image_3 = GUI:Image_Create(FrameLayout, "Image_3", 620, 350, "res/custom/npc/12tmfw/t3.png")
	...
	local no_select_text = GUI:Text_Create(FrameLayout, "no_select_text", 698, 350, 18, "#ffffff", [[未选择符文]])
	...
	-- Create player_name
	local player_name = GUI:Text_Create(FrameLayout, "player_name", 735, 482, 18, "#ffff00", [[]])
