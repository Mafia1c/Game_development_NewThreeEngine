...
function ui.init(parent, __data__, __update__)
	...
	-- Create FrameBG
	local FrameBG = GUI:Image_Create(FrameLayout, "FrameBG", 1, -3, "res/custom/npc/50zh/bg6.png")
	...
	-- Create dazao_btn
	local dazao_btn = GUI:Button_Create(FrameLayout, "dazao_btn", 630, 57, "res/custom/npc/32mj/anb1.png")
	...
	GUI:Button_setTitleText(dazao_btn, [[晋升称号]])
	...
	-- Create model_node
	local model_node = GUI:Node_Create(FrameLayout, "model_node", 320, 243)
	GUI:setTag(model_node, 0)

	-- Create effect_node
	local effect_node = GUI:Node_Create(FrameLayout, "effect_node", 320, 371)
	GUI:setTag(effect_node, 0)

	-- Create has_img
	local has_img = GUI:Image_Create(FrameLayout, "has_img", 634, 73, "res/custom/npc/50zh/yhd.png")
	GUI:setAnchorPoint(has_img, 0.00, 0.00)
	GUI:setTouchEnabled(has_img, false)
	GUI:setTag(has_img, 0)
	GUI:setVisible(has_img, false)
