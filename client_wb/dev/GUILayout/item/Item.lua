local __GD_Scale = SL:GetValue("GAME_DATA", "itemSacle")
local __GD_Style = SL:GetValue("GAME_DATA", "goods_item_star_styleid") or ""
local __GD_PowerDir = SL:GetValue("GAME_DATA", "itemPowerTagDir") or 0
...
function Item:InitUI()
    local bgImg = self._ui["Image_bg"]
    local panel = self._ui["Panel_bg"]
    local size = GUI:getContentSize(bgImg)
    GUI:setContentSize(panel, size.width, size.height)
    GUI:setContentSize(self._ui["Panel_extra"], size.width, size.height)
...
function Item:Cleanup()
    ...
    GUI:setPosition(ui["Image_bg"], size.width * 0.5, size.height * 0.5)
    ...
    GUI:setPosition(ui["Node_lock"], size.width * 0.5, size.height * 0.5)
    GUI:setPosition(ui["Panel_extra"], size.width * 0.5, size.height * 0.5)
    GUI:setPosition(ui["Node_sfx_power"], size.width * 0.5, size.height * 0.5)
    GUI:setPosition(ui["Image_choosTag"], size.width * 0.5, size.height * 0.5)
    GUI:setPosition(ui["Node_left_top"], size.width * 0.5, size.height * 0.5)
    GUI:setPositionX(ui["Node_needNum"], size.width)
...
function Item:InitData(data)
    ...
    local itemData = data.itemData or SL:GetValue("ITEM_DATA", data.index)
...
function Item:SetCount(count, format)
    ...
    local str = SL:GetSimpleNumber(count)
    if count % 1000 == 0 then
        str = self:ConvertNumberString(str)
    end
    GUI:Text_setString(ui["Text_count"], str)
end

function Item:ConvertNumberString(str)
    -- 直接匹配整数部分和"万 亿"字
    local result = string.match(str, "(%d+)%.?%d*万")
    if result then
        return result .. "万"
    end
    local result2 = string.match(str, "(%d+)%.?%d*亿")
    if result2 then
        return result2 .. "亿"
    end    
    return str
...
function Item:SetNeedNum(needNum, format)
    ...
    -- 总数量
    local totalNum = tonumber(SL:GetValue("MONEY", self._index)) or 0
...
function Item:SetItemLock()
    ...
    local isBind = SL:GetValue("ITEM_IS_BIND", self._itemData)
    if not isBind then
        if imgLock then
...
function Item:SetFullTips()
    ...
    local promptData = BagData.GetPromptGameData()
...
function Item:SetEquipRedMask()
    ...
    if SL:GetValue("ITEMTYPE", itemData) ~= ItemType.Equip then
        return false
    end
    ...
    elseif from == ItemFrom.BAG then
        ...
        if not IsCanUse and SL:GetValue("HERO_IS_ALIVE") then
            IsCanUse = SL:CheckItemUseNeed_Hero(itemData).canUse
        end
    end
