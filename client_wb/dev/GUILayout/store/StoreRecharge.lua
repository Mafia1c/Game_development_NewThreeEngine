...
function StoreRecharge.main()
    local parent = GUI:GetLayerOpenParam().parent
    
    StoreRecharge._isPC = SL:GetValue("IS_PC_OPER_MODE")
...
function StoreRecharge.UpdateExchange()
    ...
    -- 有货币信息
    local product = SL:GetValue("RECHARGE_PRODUCT_BY_ID", StoreRecharge._productID)
    if not product then
        GUI:Text_setString(StoreRecharge._ui.Text_exchange, "")
        return nil
    end
    ...
    local currencyStr = string.format("元=%s", tonumber(exchange/2)) .. product.currency_name
    -- 额外赠送
    local extraStr = ""
...
function StoreRecharge.InitInput()
    ...
    local inputMin = tonumber(SL:GetValue("GAME_DATA", "minRecharge")) and 10 or 10
    local inputMax = tonumber(SL:GetValue("GAME_DATA", "maxRecharge")) or 99999999
...
function StoreRecharge.onClickSubmitPay(channel)
    ...
    -- 是否有商品
    local product = SL:GetValue("RECHARGE_PRODUCT_BY_ID", StoreRecharge._productID)
    if not product or StoreRecharge._productID~="2" then
...
-- 界面关闭回调
function StoreRecharge.OnClose()
    StoreRecharge.UnRegisterEvent()
    SL:UnAttachTXTSUI({
        index = SLDefine.SUIComponentTable.Recharge
    })
end
