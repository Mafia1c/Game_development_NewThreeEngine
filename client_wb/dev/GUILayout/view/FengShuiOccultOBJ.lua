local FengShuiOccultOBJ = {}
FengShuiOccultOBJ.Name = "FengShuiOccultOBJ"
FengShuiOccultOBJ.cfg =GetConfig("FengShuiOccultCfg")
FengShuiOccultOBJ.suit_cfg =GetConfig("FengShuiOccultSuitCfg")
function FengShuiOccultOBJ:main(data)
	local parent = GUI:Win_Create(self.Name, 0, 0, 0, 0, false, false, true, false)
    self._parent = parent
    GUI:LoadExport(parent, "npc/FengShuiOccultUI", function () end)
    self.ui = GUI:ui_delegate(parent)
    GUI:addOnClickEvent(self.ui.close_btn,function ()
    	ViewMgr.close("FengShuiOccultOBJ")
    end)
    self.data = SL:JsonDecode(data) or {}
    self:UpdaViewInfo()

    GUI:addOnClickEvent(self.ui.unlock_btn,function ()
    	SendMsgCallFunByNpc(0,"FengShuiOccult","UnLockClick")
    end)
...
function FengShuiOccultOBJ:UpdaViewInfo()
...
function FengShuiOccultOBJ:GetActiveNum()
    local num = 0
    local data = GameData.GetData("T_occult_level",true) or {}
...
function FengShuiOccultOBJ:GetBtnRed()
	if self:GetActiveNum() >= #self.cfg then
		return false
	end
	return SL:Get_ITEM_COUNT("罗盘") > 0
end
return FengShuiOccultOBJ